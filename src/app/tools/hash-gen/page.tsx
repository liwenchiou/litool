  ...
  return (
    <div ref={containerRef} className="min-h-full p-6 sm:p-12 md:p-20 max-w-6xl mx-auto w-full flex flex-col">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
        ...
        {/* 左側：純文字輸入 */}
        <div className="tool-panel flex flex-col bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden focus-within:border-zinc-500 focus-within:ring-1 focus-within:ring-zinc-500 transition-all shadow-xl h-[500px]">
          <div className="flex justify-between items-center px-4 py-3 border-b border-zinc-800 bg-zinc-800/40">
            <span className="text-sm font-medium text-zinc-200">Input Text</span>
            <button 
        ...
        <div className="tool-panel flex flex-col gap-4">
          {[
            ...
            <div key={item.id} className="flex flex-col bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden transition-all shadow-md group hover:border-zinc-600">
              <div className="flex justify-between items-center px-4 py-2.5 border-b border-zinc-800 bg-zinc-800/40">
                <span className="text-sm font-medium text-zinc-200">{item.name}</span>
                <button 
                  onClick={() => copyToClipboard(item.value, item.id)}
                  className="text-xs flex items-center gap-1.5 text-zinc-400 hover:text-white transition-colors bg-zinc-800 px-2 py-1 rounded-md hover:bg-zinc-700 border border-zinc-700/50"
                >
                  {copied === item.id ? (
                    <>
